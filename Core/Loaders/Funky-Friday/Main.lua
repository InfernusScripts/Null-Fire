...
local function hitNote(note, scrollSpeed, key)
	...
	for _, v in note:GetChildren() do
		if v and v.Size ~= UDim2.fromScale(1, 1) then
			time = v.Size.Y.Scale / scrollSpeed
			break
		end
	end
...
local function rollChance()
	...
	while sum >= 0 do
		...
		if chances[i][2] ~= 0 then
			sum -= math.random(1, chances[i][2])
		end
	end
...
local function calculate(note, renders, dontUseCache)
	...
	local distance = math.abs(getDistance(originalY, newY))
	local scrollSpeed = math.round(math.clamp(appendScrollSpeed(distance / deltaTime, dontUseCache), 0, 17) * 100) / 100

	local distanceToTarget = math.abs(getDistance(newY, -1.5 + ((17 - scrollSpeed) / 14)))
	local timeToReachTarget = (distanceToTarget / scrollSpeed)
	
	return timeToReachTarget, scrollSpeed
end

local function calculateMethod(note, laneIndex)
	local adjustValue = offsets[rollChance()]
	local timeToReachTarget, scrollSpeed = calculate(note, vals.CalculateRenders)

	task.wait(timeToReachTarget + adjustValue)
...
local function rapidDistanceCheckMethod(note, laneIndex)
	local adjustValue = offsets[rollChance()]
	local timeToReachTarget, scrollSpeed = math.huge, 0
	
	repeat
		timeToReachTarget, scrollSpeed = calculate(note, vals.RapidRenders, true)
	until timeToReachTarget <= 0.2 or not note or not note.Parent
...
page:AddLabel({Caption = "Works best with downscroll + 4x note speed"})
page:AddSeparator()
...
local methods = {"Calculate [Least laggy + Accurate]", "Rapid checks [Best if you have low FPS]", "Combined [Calculate + Rapid; The best one]"}
...
page:AddSlider({Caption = "Random note hold duration", Min = 0, Max = 250, Step = 1, Default = 0, Callback = function(val)
	...
	return val ~= 0 and "From -" .. val .. " to +" .. val .. " ms" or "0 ms"
end})
...
page:AddSeparator()
page:AddLabel({Caption = "Autoplay method \"Combined\":"})
