...
game:GetService("RunService").RenderStepped:Connect(function()
	...
	if not respawning and realCharacter then
		...
		if settings.Bypass then
			...
			if pp and (realCharacter:GetPivot().Position - pp.Position).Magnitude >= 128 and not settings.AntiTeleport then
				fakeChar:PivotTo(CFrame.new(realCharacter:GetPivot().Position))
			end

			realCharacter:PivotTo((fakeChar:GetPivot() + (fakeHrp.CFrame.XVector * settings.Spoofing.Position.X) + (fakeHrp.CFrame.YVector * settings.Spoofing.Position.Y) + (fakeHrp.CFrame.ZVector * settings.Spoofing.Position.Z)) * (CFrame.Angles(math.rad(settings.Spoofing.Rotation.X), math.rad(settings.Spoofing.Rotation.Y), math.rad(settings.Spoofing.Rotation.Z))))
		else
			if pp and (realCharacter:GetPivot().Position - pp.Position).Magnitude >= 128 and settings.AntiTeleport then
